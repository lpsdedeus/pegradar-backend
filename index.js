...
app.get('/api/oportunidades', async (_, res) => {
  ...
  // 1) Oráculo USDC (Chainlink/RedStone)
  let usdcPrice = 1.0;
  try {
    const oracle = await axios.get('https://api.redstone.finance/prices?symbol=USDC');
    usdcPrice = oracle.data[0]?.value || 1.0;
  } catch (err) {
    console.error('RedStone error:', err.message);
  }

  // 2) Curve Finance pools
  try {
  ...
  try {
    ...
    console.error('KyberSwap error:', err.message);
  }

  // Sempre devolve algo, mesmo que vazio
